...
3. Put special attention and tripple check all the API ROLE checks, DB access check, license checks, quota checks, other conditions in all the "Scenarios". This is key focus
4. Put special atteantion and double check you mention properly that soft deleted items are used or not used in every scenario
5. Put special atteantion and double check all the outbound communications are well described (sync, async, timeout, error management)
6. Put special atteantion and double check all the timeouts and limitations mentioned as well, with references to facts
7. Put special atteantion and double check all the events and audit logs are mentioned in all the "Scenarios"
