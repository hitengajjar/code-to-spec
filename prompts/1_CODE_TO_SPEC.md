...
7. When you describe behavioural scenario in ORIG_BEHAVIOUR.md file, for every fact you identified, put a reference to the fact like [F01.123], so that later I would be able to map some of the facts onto the new technological stack and, say, change the way I implement access check on DB layer, or change the way I do logging, or change the way I
...
3. tripple check all the API ROLE checks, DB access check, license checks, quota checks, other conditions in all the "Scenarios". This is key focus
